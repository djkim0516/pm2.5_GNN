...
class GraphGNN(nn.Module):
    ...
    def forward(self, x):
        ...
        node_src = x[:, edge_src]       #history data
        ...
        self.edge_attr_ = self.edge_attr[None, :, :].repeat(node_src(0), 1, 1)  #None을 쓰면 해당 차원을 한개로 인식함
        city_dist = self.edge_attr[:, :, 0]
        city_direc = self.edge_attr[:, :, 1]
        ...
        out_add = scatter_add(out, edge_target, dim=1, dim_size=x.size(1))          #더해진 미세먼지
        out_sub = scatter_add(out.neg(), edge_src, dim=1, dim_size=x.size(1))       #나간 미세먼지
