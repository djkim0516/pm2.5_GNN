...
from geopy.distance import geodesic         #geo data
from metpy.units import units               #meteo data
...
class Graph():
    def __init__(self):
        self.dist_thres = 3         #300km?
        self.alti_thres = 1200      #1200m?
    ...
    def _gen_edges(self):
        ...
        dist = distance.cdist(coords, coords, metric='euclidean') #compute distance
        adj = np.zeros((self.node_num, self.node_num), dtype=np.uint8)
        adj[dist <= self.dist_thres] = 1        #threshold보다 작은 애들은 인접
        assert adj.shape == dist.shape
        dist = dist * adj
        edge_index, dist = dense_to_sparse(torch.tensor(dist))  #용량 축소
