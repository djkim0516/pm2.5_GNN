...
from util import config, file_dir, nodename
...
def get_metric(predict_epoch, label_epoch):
    haze_threshold = 75                                 #최소 75는 되야 미세먼지 있다고 판단?
    predict_haze = predict_epoch >= haze_threshold      #미세먼지 예측 true 위치
    predict_clear = predict_epoch < haze_threshold      #clean 예측 true 위치
    label_haze = label_epoch >= haze_threshold          #실제 true 위치
    label_clear = label_epoch < haze_threshold          #실제 clean 위치
    hit = np.sum(np.logical_and(predict_haze, label_haze))      #둘다 true 인곳
...
def test(test_loader, model):
    ...
    for batch_idx, data in enumerate(test_loader):
        ...
        #normalized -> to original data value 역변환
